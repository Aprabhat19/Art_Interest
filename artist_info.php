...
<body>
    ...
    $user_id = $_GET['id'];
        ...
        $query_category="SELECT user_imagepath,user_fname, user_lname,user_mname, user_gender, user_house_number, user_street,  user_state,user_city, user_contact FROM user WHERE user_id = '$user_id'";
        $result_category = mysqli_query($conn,$query_category);

        while($row=mysqli_fetch_array($result_category)){

            echo ' <h2 class="headspace"> '.$row['user_fname'].'\'s Info <hr class="hr" style="border-bottom: 2px solid #2a8048;"></h2>

                  <p></p>

                  <div class="backgd"><a  href= "pictures/profile/'.$row['user_imagepath'].'"> <img class="photo1" src="pictures/profile/'.$row['user_imagepath'].'"></a>

                  <p class = "info-title">Full Name: <br>Gender: <br>State: <br>City: <br>Contact Info: </p></div>

                  <p class = "info-content">'.$row['user_fname'].' '.$row['user_mname'].' '.$row['user_lname'].'<br>'.$row['user_gender']. '<br> '.$row['user_state'].'<br>'.$row['user_city'].' <br>'.$row['user_contact'].'</p></div> ';
        ...
        while ($row2=mysqli_fetch_array($result_category2)) {
                    echo '<h2 class="headspace2"> '.$row2['user_fname'].'\'s Artworks <hr class="hr" style="border-bottom: 2px solid #2a8048;"></h2>';
        }
